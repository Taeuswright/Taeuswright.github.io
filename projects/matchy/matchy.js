...
function addAnimals(ans, id) {
  ...
  for (var i = 0, len = ans.length; i < len; i++) {
    var mu =
      ...
      ans[i].name +
      "</h2>" +
      '<h3><b>Species:</b><span class="anSpecies"> ' +
      ans[i].species +
      "</span></h3>" +
      "<h3><b>Friends:</b></h3>" +
      "<ul>";
...
$(function() {
  ...
  if (window.search && typeof window.search === "function") {
    ...
    $("#searchBtn").on("click", function() {
      ...
      if (an === undefined || an === null) {
        $("#results")
          .html("")
          .html("No animals with that name found!");
      } else {
  ...
  $("body").on("click", ".editBtn", function() {
    ...
    $(this)
      .siblings("h2.anName")
      .replaceWith(
        `<h2><input type="text" class="anNameInp" value="${
          window.___editingAnimal.name
        }"></h2>`
      );
    $(this)
      .parents(".animal")
      .find("span.anSpecies")
      .replaceWith(
        `<span><input type="text" class="anSpeciesInp" value="${
          window.___editingAnimal.species
        }"></span>`
      );
  });
  ...
  if (window.add && typeof window.add === "function") {
    ...
    $("body").on("click", "#createBtn", function() {
      var new_name = $(this)
        .siblings("#newNameInp")
        .val();
      var new_species = $(this)
        .siblings("#newSpeciesInp")
        .val();
      $(this)
        .siblings("#newNameInp")
        .val("");
