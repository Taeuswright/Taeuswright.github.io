    ...
    let create = {
      onTop: function(position, delay) {
        if (position < 0 || position > game.world.width) {
          throw new Error(
            `You are trying to place a cannon off the stage at ${position}, this is not allowed!`
          );
        }
        ...
        let tween = game.add
          .tween(projectile)
          .to(
            { y: game.world.height + projectile.height },
            2000,
      ...
      onBottom: function(position, delay) {
        ...
        var cannon = game.cannon.create(
          position,
          game.world.height - 72,
          assetKeyCannon
        );
        cannon.anchor.setTo(0.5, 0.5);
        let projectile = game.projectile.create(
          cannon.x,
          cannon.y,
          assetKeyProjectile
        );
      ...
      onLeft: function(position, delay) {
        ...
        let tween = game.add
          .tween(projectile)
          .to(
            { x: game.world.width + projectile.height },
            2000,
            Phaser.Easing.Bounce.Out,
            true,
            delay || 0,
