(function(window) {
  ...
  window.opspark.makeHalle = function(
    spritesheet,
    particleManager,
    showHitZones
  ) {
    ...
    p.duck = function() {
      setAsset(_duck);
      tweenHitForAction(
        halle.y + halle.getBounds().height / 4,
        null,
        100,
        150,
        220
    ...
    function kickUpDust() {
      _dust.emit(
        { x: halle.x + 8, y: halle.y + halle.getBounds().height },
        0.3
      );
    }
    ...
    function tweenHitForAction(toY, toX, timeIn, timeOut, between, toRotation) {
      createjs.Tween.get(hitzones)
        .to(
          { y: toY, x: toX || halle.x, rotation: toRotation || halle.rotation },
          timeIn
        )
        .wait(between || 0)
    ...
    _dust = particleManager.makeEmitter(
      1,
      3,
      null,
      new Proton.Velocity(
